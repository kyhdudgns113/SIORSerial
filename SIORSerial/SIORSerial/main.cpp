...
void study_data() {
	...
	//	printf("b%d ", read_end);
		if (read_end) {
			NEXT_STATE[previous_angle][previous_speed].addData(max_angle);
			present_angle = previous_angle;
		}
		else {
			present_angle = input_signal;
	...
	//	printf("(%4d, %4d, %4d) : %4d", previous_angle, previous_speed, present_speed, present_angle);
		double best_value = 999999;
		int best_i = -1;
...
void using_data() {
	...
	while (ser.isOpen()) {
		...
		//	각도의 범위가 벗어났을때, 다시 이전으로 돌아와서 다른 값을 대입하여 학습한다.
		if (read_end) {
			printf("나는 잠시 발을 헛디딘 것일 뿐이다.\n");
			break;
		}
		else {
			present_angle = input_signal;
		}
		int real_previous_angle = 4 * (previous_angle - 45);
		int real_present_angle = 4 * (present_angle - 45);
		int real_present_speed = 4 * (present_speed - 64);
		int real_speed = 4 * (previous_speed - 64);
		printf("(%4d, %4d, %4d) -> %4d\n", real_previous_angle, 
			real_speed, real_present_speed, real_present_angle);
		double best_value = 999999;
		int best_i = -1;
